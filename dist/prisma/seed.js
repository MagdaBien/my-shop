...
async function seed() {
    ...
    await Promise.all(getClients().map((client) => {
        return db.client.create({ data: client });
    }));
    await Promise.all(getOrders().map(({ productId, clientId, ...orderData }) => {
    ...
    await Promise.all(getPhotos().map(({ productId, ...photoData }) => {
